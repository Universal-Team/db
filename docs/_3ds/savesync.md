---
author: Cool Guy
avatar: https://avatars.githubusercontent.com/u/46397838?v=4
categories:
- utility
color: '#579ac0'
color_bg: '#3a6680'
created: '2025-11-16T04:15:26Z'
description: Allows you to copy saves from one 3DS or emulator to another easily
download_page: https://github.com/coolguy1842/SaveSync/releases
downloads:
  SaveSync.3dsx:
    size: 2072912
    size_str: 1 MiB
    url: https://github.com/coolguy1842/SaveSync/releases/download/v1.0.0/SaveSync.3dsx
  SaveSync.cia:
    size: 1300416
    size_str: 1 MiB
    url: https://github.com/coolguy1842/SaveSync/releases/download/v1.0.0/SaveSync.cia
github: coolguy1842/SaveSync
icon: https://raw.githubusercontent.com/coolguy1842/SaveSync/master/assets/icon.png
image: https://raw.githubusercontent.com/coolguy1842/SaveSync/master/assets/banner.png
image_length: 3416
layout: app
license: gpl-3.0
license_name: GNU General Public License v3.0 or later
qr:
  SaveSync.cia: https://db.universal-team.net/assets/images/qr/savesync-cia.png
source: https://github.com/coolguy1842/SaveSync
stars: 1
systems:
- 3DS
title: SaveSync
unique_ids:
- '0x49FE2'
update_notes: <p dir="auto">All planned features implemented, main changes from here
  on are likely to be bug fixes, and server protocol upgrades.</p>
updated: '2025-12-12T07:26:48Z'
version: v1.0.0
version_title: v1.0.0
---
