---
author: Unitendo
avatar: https://avatars.githubusercontent.com/u/241876109?v=4
categories:
- app
color: '#2d8d97'
color_bg: '#267780'
created: '2026-03-14T01:11:35Z'
description: ' Real Time Chatting for the 3DS, Wii, Wii U, and Web'
download_page: https://github.com/Unitendo/aurorachat/releases
downloads:
  aurorachat.3dsx:
    size: 5000912
    size_str: 4 MiB
    url: https://github.com/Unitendo/aurorachat/releases/download/v0.6.0.9/aurorachat.3dsx
  aurorachat.cia:
    size: 5542848
    size_str: 5 MiB
    url: https://github.com/Unitendo/aurorachat/releases/download/v0.6.0.9/aurorachat.cia
github: Unitendo/aurorachat
icon: https://raw.githubusercontent.com/Unitendo/aurorachat-3ds/main/meta/icon.png
image: https://raw.githubusercontent.com/Unitendo/aurorachat-3ds/main/meta/banner.png
image_length: 28629
layout: app
license: mit
license_name: MIT License
llm_generation: unknown
qr:
  aurorachat.cia: https://db.universal-team.net/assets/images/qr/aurorachat-cia.png
source: https://github.com/Unitendo/aurorachat
stars: 18
systems:
- 3DS
title: aurorachat
unique_ids:
- '0xBAFD2'
update_notes: <p dir="auto">In the last update, you could get banned on 3DS. Now you
  can make an appeal... on 3DS?? Maybe? I didn't test if our website works on 3DS.
  you might need to use like your phone or something.</p>
updated: '2026-06-17T15:39:17Z'
version: v0.6.0.9
version_title: 'v0.6.0.9: The Ban Appeal update'
---
A safer chatting app for the Nintendo 3DS line of systems.