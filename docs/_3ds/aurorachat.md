---
author: mii-man, Virtualle, and others
avatar: https://avatars.githubusercontent.com/u/136441553?v=4
categories:
- app
color: '#2d8d97'
color_bg: '#267780'
created: '2025-10-12T03:42:15Z'
description: Real Time Chatting for the 3DS, Wii and Wii U
download_page: https://github.com/mii-man/aurorachat/releases
downloads:
  aurorachat.3dsx:
    size: 928120
    size_str: 906 KiB
    url: https://github.com/mii-man/aurorachat/releases/download/v0.4.5/aurorachat.3dsx
  aurorachat.cia:
    size: 1209280
    size_str: 1 MiB
    url: https://github.com/mii-man/aurorachat/releases/download/v0.4.5/aurorachat.cia
github: mii-man/aurorachat
icon: https://raw.githubusercontent.com/mii-man/aurorachat/main/meta/icon.png
image: https://raw.githubusercontent.com/mii-man/aurorachat/main/meta/banner.png
image_length: 28629
layout: app
license: mit
license_name: MIT License
qr:
  aurorachat.cia: https://db.universal-team.net/assets/images/qr/aurorachat-cia.png
source: https://github.com/mii-man/aurorachat
stars: 16
systems:
- 3DS
title: aurorachat
unique_ids:
- '0xBAFD2'
update_notes: '<p dir="auto">Changelog:</p>

  <ul dir="auto">

  <li>Add better errors</li>

  <li>Fix major exploit (mostly server-side, add indication on client)</li>

  <li>etc.</li>

  </ul>'
updated: '2026-03-08T23:21:18Z'
version: v0.4.5
version_title: 'v0.4.5: Half-baked Build'
wiki: https://github.com/mii-man/aurorachat/wiki
---
A safer chatting app for the Nintendo 3DS line of systems.