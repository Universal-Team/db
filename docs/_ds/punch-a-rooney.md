---
author: Sean David
avatar: https://avatars.githubusercontent.com/u/200457797?v=4
categories:
- game
color: '#e82b2b'
color_bg: '#801818'
created: '2026-07-30T04:50:38Z'
description: The homebrew boxing game made for the Nintendo DS!  Play as Rooney "Punchstache"
  Bafutto, a 22-year-old Italian boxer who quits his job as a plumber. Fight against
  five fighters, become champion, then defend your title!
download_page: https://github.com/seaners42/Punch-A-Rooney/releases
downloads:
  Punch-A-Rooney.nds:
    size: 13714432
    size_str: 13 MiB
    url: https://github.com/seaners42/Punch-A-Rooney/releases/download/1.0.1/Punch-A-Rooney.nds
github: seaners42/Punch-A-Rooney
icon: https://raw.githubusercontent.com/seaners42/Punch-A-Rooney/main/icon.png
image: https://raw.githubusercontent.com/seaners42/Punch-A-Rooney/main/banner.png
image_length: 131593
layout: app
license: apache-2.0
license_name: Apache License 2.0
llm_generation: 'no'
qr:
  Punch-A-Rooney.nds: https://db.universal-team.net/assets/images/qr/punch-a-rooney-nds.png
screenshots:
- description: Cornerman
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/cornerman.png
- description: Freeplay
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/freeplay.png
- description: Gameplay1
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/gameplay1.png
- description: Gameplay2
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/gameplay2.png
- description: Gameplay3
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/gameplay3.png
- description: Preview
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/preview.png
- description: Title
  url: https://db.universal-team.net/assets/images/screenshots/punch-a-rooney/title.png
source: https://github.com/seaners42/Punch-A-Rooney
stars: 1
systems:
- DS
title: PUNCH-A-ROONEY!
update_notes: '<h2 dir="auto">Change Notes</h2>

  <ul dir="auto">

  <li>Fixed referee behavior</li>

  <li>Fixed "Gallery" when using touchscreen to navigate</li>

  <li>Guide button is made more obvious to new players</li>

  </ul>'
updated: '2026-08-07T03:11:38Z'
version: 1.0.1
version_title: Punch-A-Rooney v1.0.1
---
### Quick Guide
## Career
Five fighters stand in your way before you can claim the belt, once you become champ you can defend your title against the same fighters!

###Multiplayer
Punch-A-Rooney! supports Local Multiplayer via two Nintendo DS consoles where both fighters can choose a boxer of their choice to personally duke it out!

*Enter Local on both consoles
*Host on one console, then join on the other
*As a host, press A to begin editing your match information
*Press start on the last screen if you're the host to start
*If you are not the host, choose your character quick before the host starts!

Host will always fight out of the left corner, while their opponent will fight from the right.